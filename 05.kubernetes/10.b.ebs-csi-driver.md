...
### Step-1: Check the OIDC Connector details of cluster
```bash
...
Check with OIDC Provider ID which we got earlier. If it doesn't exist we have to create it.
```bash
aws iam list-open-id-connect-providers
```

### Step-2: Register the OIDC Provider in IAM using CLI
```bash
eksctl utils associate-iam-oidc-provider \
  --cluster $CLUSTER_NAME --region $REGION --approve=false
```
...
## replace the whole ARN, otherwise it will not work --> Use gitbash preferred
```
...
> `Note`: check whether the variables are appended correctly into file or not, it depends on your shell (gitbash preferred)
...
(Optional - The addon will create service account and attach role automatically by using the `eksctl` command)
