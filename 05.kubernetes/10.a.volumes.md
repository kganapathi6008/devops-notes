          ...
          requiredDuringSchedulingIgnoredDuringExecution:
            ...
            - matchExpressions:
              - key: kubernetes.io/hostname
                operator: In
                values:
                - ip-192-168-102-53.ap-south-2.compute.internal
      containers:
      - name: mongo
        image: mongo
        env:
...
## 📁 3.1. PersistentVolume + PersistentVolumeClaim (Static Provisioning)
...
## 📁 3.2. PersistentVolumeClaim with Dynamic Volume Provisioning
...
```bash
kubectl get sc      ## to get the storage class details
```
...
- Allow sometime (usually in 10 minutes) for the new node to create and the volume is in `attached` state for the new node. Then the pod will be in `running` state and attached to the `pvc`.
