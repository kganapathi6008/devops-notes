...
### **Manually Annotate the Change Cause**

If you've already performed an update (like `set image`), you can manually add or update the change cause:

```sh
kubectl annotate deployment nginx-deployment kubernetes.io/change-cause="Updated image to nginx:1.23 manually" --overwrite
```

### **Inline Update with Change-Cause Annotation**

Kubernetes does not support directly setting a custom change cause inline with `kubectl set image`. However, you can chain commands to achieve the same result:

```sh
kubectl set image deployment nginx-deployment nginx-container=nginx:1.23
kubectl annotate deployment nginx-deployment kubernetes.io/change-cause="Custom change: nginx to 1.23" --overwrite
```
...
To automatically record the command used to make changes:

```sh
kubectl set image deployment nginx-deployment nginx-container=nginx:1.23 --record
```

This stores the command itself as the change cause.
